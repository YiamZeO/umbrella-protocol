...
2. В фазе Handshake (record types `0x14` CCS, `0x15` Alert, `0x16` Handshake) каждый inner TLS record оборачивается в Vision-фрейм перед отправкой в туннель:
   ...
   ```
3. При Application Data record (`0x17`) — клиент остается в режиме Vision-фреймов:
...
4. **Random Padding:** продолжает ломать статистические сигнатуры длин пакетов.
