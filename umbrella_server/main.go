...
func main() {
	...
	if cfg.PrivateKey == "" {
		...
		b, err := base64.StdEncoding.DecodeString(cfg.PrivateKey)
		if err != nil || len(b) != 32 {
	...
	if cfg.ServerNames != "" {
		for _, name := range strings.Split(cfg.ServerNames, ",") {
	...
	if err != nil {
		log.Fatalf("listen :%s: %v", cfg.Port, err)
	}
	log.Printf("Umbrella/REALITY server on :%s (fallback → %s)", cfg.Port, dest)
