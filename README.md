...
- Для реалистичности запросы содержат стандартные заголовки браузера (User-Agent), а часть тела ответа (до 512 КБ) реально вычитывается из сети, создавая физический трафик.
...
- На Android использую связку `ReTerminal` + `Clash Meta`. `ReTerminal` нужен для запуска клиента\менеджера. В `Clash Meta` прокидывается конфиг файл такой же как для `Clash Verge`.
