...
- **DNS**: встроенный локальный DNS-сервер (dnsmasq-подобный) с кэшированием (IP→hostname + revert) для корректной работы bypass
...
### Windows
Относительно просто
- Можно запустить как обычной socks5 сервер и подключать к нему приложения через настройки или расширение браузера. 
...
После всех настроек так же понадобится отдельное приложение для включения VPN и перенаправления трафика на `Umbrella client`. Работоспособность проверена с [Clash Meta](https://github.com/MetaCubeX/ClashMetaForAndroid), [sing-box](https://github.com/SagerNet/sing-box-for-android), [VProxid](https://github.com/ProxidBean/VProxid). Можно использовать любые другие приложения которые смогут соединиться с `Umbrella client` по socks5
