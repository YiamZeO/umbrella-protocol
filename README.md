...
- **Низкая скорость в Torrent**: Это особенность протокола uTP (LEDBAT), он уступает канал другим приложениям. Попробуйте Hysteria для максимальной скорости.
- **Ошибка SSL в XTLS**: Проверьте, что `sni` на клиенте совпадает с `server-names` на сервере.
