...
2. Сервер принимает этот поток и запускает **phase engine**: бесконечный цикл, который случайно выбирает следующую фазу (≠ предыдущей) и её продолжительность, отправляет клиенту 12-байтное сообщение с полными параметрами, спит выбранную длительность.
3. Клиент получает параметры и применяет лимиты у себя. Локальный таймер на `duration_ms` гарантирует выход из фазы даже при потере соединения с сервером — по истечении таймера throttle сбрасывается в fallback (без ограничений).

**Протокол сообщения (12 байт, big-endian):**

```
[0:4]  duration_ms  uint32   — длительность фазы в миллисекундах
[4:8]  down_mbps    float32  — лимит сервер→клиент
[8:12] up_mbps      float32  — лимит клиент→сервер
```
...
Между каждой парой фаз сервер выдерживает случайную паузу **1–10 секунд** (throttle на это время сбрасывается в fallback), после чего активирует следующую фазу.
...
4. **Инструменты изменения поведения трафика и размытия его паттернов (Shaper + Decoy Traffic):** Оба инструмента могут использоваться как по отдельности так и вместе чтобы создать непредсказуемый паттерн трафика и затруднить для DPI статистический анализ.
...
- **Включение `--decoy-traffic` может увеличить потребление энергии и трафика**
