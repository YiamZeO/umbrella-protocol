...
*   **База:** QUIC (UDP) с BBR.
*   **Маскировка:** Маскарад под HTTPS + Random padding на уровне пакетов.
    * В Umbrella Маскарад модифицирован до уровня XTLS Reality с защитой от probbing.
    * Используется патч библиотеки hysteria, который добавляет Random padding на уровне пакетов
...
*   **База:** TCP (TLS 1.3).
*   **Маскировка:** Кража личности (Reality) легитимного сайта + "тонировка" Vision
    * Vision в Umbrella модифицирован для постоянной маскировки как TLS-in-TLS сигнатур так и всего полезного трафика приложений засчет постоянного Random padding.
...
*   **Маскировка:** Поведенческая имитация торрент-клиента + White Noise (запросы к трекерам) + Random padding.
*   **Особенности:** Почти невозможно заблокировать, не сломав легитимный BitTorrent-трафик в стране.
    * Используется патч библиотеки uTP, который снимает лимиты и заставляет протокол пытаться взять максимум скорости.
