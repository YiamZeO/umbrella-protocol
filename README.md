...
**Задача.** Даже при правильном TLS fingerprint и скрытой аутентификации DPI может детектировать туннель по характеру потока: Некоторый трафик ограничиваемых ресурсов может быть почти равномерным и непрерывным. Shaper может имитировать поведение реального непостоянного браузерного трафика за счет ограничений скоростей. Эффективнее всего если изначальная форма трафика является постоянным потоком.
...
### Уровень 5 — Decoy Traffic (генерация трафика, размывающего паттерны)

**Задача.** Аналогично Shaper влияет на поведение и общий паттерн трафика, но засчет генерации фиктивного трафика и запросов. Эффективен для трафика с паттерном "всплеск-пауза-всплеск-пауза" (например, при просмотре видео на YouTube) т.к. системы DPI могут анализировать длительность пауз и объемы всплесков для статистической классификации протокола. В такие периоды буферизации туннель простаивает, что выдает характерный профиль потребления контента.

**Как работает.**

1. При активации флага `--decoy-traffic` клиент запускает независимый фоновый процесс генерации "шумового" трафика.
2. Процесс периодически отправляет HTTP GET-запросы к случайно выбранным популярным белым ресурсам (шрифты Google Fonts, популярные библиотеки с CDN, серверы статических изображений).
...
**Результат.** Изначальный анализируемый паттерн трафика заполняются постоянной сетевой активностью и размывается, сглаживая гистограмму распределения пакетов по времени. Что критически усложняет эвристический анализ сессий со стороны систем глубокого инспекции трафика.
...
4. **Инструменты изменения поведения трафика и размытия его паттернов (Shaper + Decoy Traffic):** Оба инструмента могут использоваться как по отдельности так и вместе чтобы создать непредсказуемую природу трафика и затруднить для DPI статистический анализ и формирование профиля потока данных.
